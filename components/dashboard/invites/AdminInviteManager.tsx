...
const InviteActionButtons = ({
  ...
  const inviteId = invite._id as string;
  const sendMode = invite.status === "draft" ? "send" : "resend";
  const isRowBusy = (prefix: string) =>
    isPending && activeAction === `${prefix}:${inviteId}`;
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {canRevoke && (
        <Button
          ...
          onClick={() => onRevokeInvite(inviteId)}
        >
...
const AdminInviteManager = ({ currentUserId }: { currentUserId: string }) => {
  ...
  const createInvite = useMutation(
    api.lib.planEntitlements.createAdminPlanInvite,
  );
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm max-[375px]:p-3.5 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-slate-600">
            ...
            The invited person still has to sign up with the exact email address
            you enter here. Resending rotates the invite link and disables older
            links.
          </p>
        </div>
      </div>

      <div className="mt-5 grid gap-3 sm:mt-6 sm:gap-4 lg:grid-cols-[minmax(0,1fr)_180px_auto]">
        <Input
          type="email"
        ...
        <div className="mt-4 space-y-3 sm:hidden">
          {(invites ?? []).map((invite) => {
            return (
              <div
                key={invite._id}
                className="rounded-[1.25rem] border border-slate-200 bg-slate-50/70 p-3.5"
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span
                    className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-medium capitalize ${getStatusClassName(invite.status)}`}
                  >
                    {getStatusLabel(invite.status, invite.isLegacy)}
                  </span>
                </div>
                ...
                <div className="mt-3 space-y-1.5 text-xs text-slate-600">
                  ...
                  <p>Last sent: {formatDate(invite.lastSentAt)}</p>
          ...
          {invites?.length === 0 && (
            <div className="rounded-[1.25rem] border border-slate-200 bg-slate-50/70 px-4 py-6 text-center text-sm text-slate-500">
              No invites created yet.
            </div>
          )}
        </div>
